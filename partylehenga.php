...
$query = "SELECT sno,image1,image2,name,price FROM partylehenga";
        ...
        <div class="row section3 py-5 px-2" style="padding-bottom:15px;">
            ...
            if ($res->num_rows > 0) {
                while ($row = $res->fetch_assoc()) {
                    ...
                    $product_link = "page.php?category=partylehenga&id=" . urlencode($row['sno']) . "&image1=" . urlencode($row['image1']) . "&image2=" . urlencode($row['image2']) . "&name=" . urlencode($row['name']) . "&price=" . urlencode($row['price']);
