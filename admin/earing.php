...
if (isset($_GET['delete_id'])) {
    ...
    $fetchQuery = "SELECT image FROM earing WHERE sno = '$id'";
    ...
    if (mysqli_num_rows($fetchResult) > 0) {
        ...
        $deleteQuery = "DELETE FROM earing WHERE sno = '$id'";
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_POST['uid'])) {
        ...
        $fetchQuery = "SELECT image FROM earing WHERE sno = '$uid'";
        $fetchResult = mysqli_query($conn, $fetchQuery);
        if ($fetchResult && mysqli_num_rows($fetchResult) > 0) {
            ...
            $sql = "UPDATE `earing` SET `image`='$image', `name`='$name', `price`='$price' WHERE `sno`='$uid'";
        ...
        if (!empty($image) && move_uploaded_file($tmpname, $folder)) {
            $sql = "INSERT INTO `earing`(`image`,`name`,`price`) VALUES ('$image','$name','$price')";
            $result = mysqli_query($conn, $sql);
            if ($result) {
...
if (isset($_GET['uid'])) {
    $uid = $_GET['uid'];
    $editQuery = "SELECT * FROM `earing` WHERE `sno` = '$uid'";
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                            ...
                            <input type="submit" name="submit" value="<?php echo isset($editRow) ? 'Update' : 'Add Earing'; ?>">
                        </form>
                    </div>
