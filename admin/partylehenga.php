...
$query = "SELECT * FROM partylehenga";
...
if (isset($_GET['delete_id'])) {
    ...
    $fetchQuery = "SELECT image1, image2 , image3  FROM partylehenga WHERE sno = '$id'";
    ...
    if (mysqli_num_rows($fetchResult) > 0) {
        ...
        $deleteQuery = "DELETE FROM partylehenga WHERE sno = '$id'";
        if (mysqli_query($conn, $deleteQuery)) {
            $_SESSION['successMessage'] = "Record deleted successfully!";
            header("Location: partylehenga.php");
            exit;
        } else {
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_POST['uid'])) {
        ...
        $fetchQuery = "SELECT image1, image2, image3 FROM partylehenga WHERE sno = '$uid'";
        $fetchResult = mysqli_query($conn, $fetchQuery);
        if ($fetchResult && mysqli_num_rows($fetchResult) > 0) {
            ...
            $sql = "UPDATE `partylehenga` SET `image1`='$image1', `image2`='$image2', `image3`='$image3', `name`='$name', `price`='$price' ,`desc`='$desc', `shipping`='$shipping' WHERE `sno`='$uid'";
        ...
        if (!empty($image1) && move_uploaded_file($tmpname1, $folder1) && !empty($image2) && move_uploaded_file($tmpname2, $folder2) && !empty($image3) && move_uploaded_file($tmpname3, $folder3)) {
            $sql = "INSERT INTO `partylehenga`(`image1`, `image2`, `image3`, `name`, `price`, `desc`, `shipping`) VALUES ('$image1', '$image2', '$image3', '$name', '$price', '$desc', '$shipping')";
            $result = mysqli_query($conn, $sql);
            if ($result) {
...
if (isset($_GET['uid'])) {
    $uid = $_GET['uid'];
    $editQuery = "SELECT * FROM `partylehenga` WHERE `sno` = '$uid'";
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                            ...
                            <input type="submit" name="submit" value="<?php echo isset($editRow) ? 'Update' : 'Add Lehenga '; ?>">
                        </form>
                    </div>
