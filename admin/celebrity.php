...
if (isset($_GET['delete_id'])) {
    ...
    $fetchQuery = "SELECT image FROM celebrity WHERE sno = '$id'";
    ...
    if (mysqli_num_rows($fetchResult) > 0) {
        ...
        $deleteQuery = "DELETE FROM celebrity WHERE sno = '$id'";
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_POST['uid'])) {
        ...
        $fetchQuery = "SELECT image FROM celebrity WHERE sno = '$uid'";
        $fetchResult = mysqli_query($conn, $fetchQuery);
        if ($fetchResult && mysqli_num_rows($fetchResult) > 0) {
            ...
            $sql = "UPDATE `celebrity` SET `image`='$image', `name`='$name' WHERE `sno`='$uid'";
        ...
        if (!empty($image) && move_uploaded_file($tmpname, $folder)) {
            $sql = "INSERT INTO `celebrity`(`image`,`name`) VALUES ('$image','$name')";
            $result = mysqli_query($conn, $sql);
            if ($result) {
...
if (isset($_GET['uid'])) {
    $uid = $_GET['uid'];
    $editQuery = "SELECT * FROM `celebrity` WHERE `sno` = '$uid'";
