...
if (isset($_GET['delete_id'])) {
    ...
    $fetchQuery = "SELECT image FROM dupatta WHERE sno = '$id'";
    ...
    if (mysqli_num_rows($fetchResult) > 0) {
        ...
        $deleteQuery = "DELETE FROM dupatta WHERE sno = '$id'";
        if (mysqli_query($conn, $deleteQuery)) {
            $_SESSION['successMessage'] = "Record deleted successfully!";
            header("Location: dupatta.php");
            exit();
        } else {
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_POST['uid'])) {
        ...
        $fetchQuery = "SELECT image FROM dupatta WHERE sno = '$uid'";
        $fetchResult = mysqli_query($conn, $fetchQuery);
        if ($fetchResult && mysqli_num_rows($fetchResult) > 0) {
            ...
            $sql = "UPDATE `dupatta` SET `image`='$image', `name`='$name', `price`='$price' WHERE `sno`='$uid'";
        ...
        if (!empty($image) && move_uploaded_file($tmpname, $folder)) {
            $sql = "INSERT INTO `dupatta`(`image`,`name`,`price`) VALUES ('$image','$name','$price')";
            $result = mysqli_query($conn, $sql);
            if ($result) {
...
if (isset($_GET['uid'])) {
    $uid = $_GET['uid'];
    $editQuery = "SELECT * FROM `dupatta` WHERE `sno` = '$uid'";
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" enctype="multipart/form-data">
                            ...
                            <input type="submit" name="submit" value="<?php echo isset($editRow) ? 'Update' : 'Add Dupatta'; ?>">
                        </form>
                    </div>
