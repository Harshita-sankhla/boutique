...
$query = "SELECT sno, image1, image2, name, price FROM arrival";
...
<body>
    ...
    <!-- section 1 Start -->
    <div class="container-fluid backimage" style="height: 220px; background-color: white;">
        <div class="overlay" style="background-color: #f7f7f7;"></div> <!-- Increased overlay opacity -->
        <div class="text" style="color: black;"> <!-- Changed text color to yellow -->
            <h1><b>NEW-ARRIVALS</b></h1> <!-- Increased font size -->
        </div>
    </div>
    <!-- section 1 End -->
        ...
        <div class="row section3 py-5 px-2" style="padding-bottom:15px;">
            ...
            if ($res->num_rows > 0) {
                while ($row = $res->fetch_assoc()) {
                    ...
                    $product_link = "page.php?category=arrival&id=" . urlencode($row['sno']) . "&image1=" . urlencode($row['image1']) . "&image2=" . urlencode($row['image2']) . "&name=" . urlencode($row['name']) . "&price=" . urlencode($row['price']);
